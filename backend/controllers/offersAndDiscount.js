...
const createOffer = async (req, res) => {
  try {
    ...
    if (
      !payload.title ||
      !payload.yourNameBusinessInstituteFirmOrganisation ||
    ...
    if (payload.selectSubCategory === "Other" && !payload.subCategoryOther) {
      return res
        .status(400)
        .json({ message: "Sub-Category Other is required when Other is selected" });
    }
    ...
    if (payload.allowCallViaPhone === true) {
      ...
      if (registeredPhone !== last10Digits) {
        ...
        if (!isVerified && req.user.role !== "ADMIN") {
          return res.status(400).json({
            message:
              "Phone number must be verified via OTP before creating offer. Please verify the phone number first.",
            status: 400,
            success: false,
            error: true,
            data: {
    ...
    // If user has posts → decrease limit
    if (user.offerPostLimit > 0 && user.role !== "ADMIN") {
      updateQuery.$inc = { offerPostLimit: -1 };
    }
    ...
    if (allowCallViaPhone === true) {
      ...
      if (registeredPhone !== last10Digits) {
        ...
        if (!isVerified && req.user.role !== "ADMIN") {
          return res.status(400).json({
            message:
              "Phone number must be verified via OTP before updating offer. Please verify the phone number first.",
            status: 400,
            success: false,
            error: true,
            data: {
