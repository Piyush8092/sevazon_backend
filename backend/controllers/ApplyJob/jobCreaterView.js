...
const getApplyedJobCreterView= async (req, res) => {
    try {  
        ...
        const result = await ApplyJob.find({job_creatorId:userId})
        ...
        const total = await ApplyJob.countDocuments({job_creatorId:userId});
        const totalPages = Math.ceil(total / limit);

        res.json({message: 'Applications retrieved successfully', status: 200, data: result, success: true, error: false, total, totalPages});
    } catch (e) {
