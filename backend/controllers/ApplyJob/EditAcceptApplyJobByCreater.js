...
const updateApplyStatusByCreater = async (req, res) => {
    try {       
        ...
        if(payload.accept_status && !['Pending', 'Accepted', 'Rejected'].includes(payload.accept_status)) {
        ...
        res.json({message: 'Application updated successfully', status: 200, data: result, success: true, error: false});
    }
    catch (e) {
