...
const LoginRout = async (req, res) => {
  try {
    ...
    if ((!payload.email && !payload.phone) || !payload.password) {
      return res
        .status(400)
        .json({ message: "Password and at least one of email or phone are required" });
    }
    ...
    if (fcmToken && deviceId) {
      ...
      existingUser.fcmTokens = existingUser.fcmTokens.filter(
        (t) => t && t.token && t.token.trim() !== ""
      );
      ...
      if (existingTokenIndex !== -1) {
        ...
        const existingDeviceIndex = existingUser.fcmTokens.findIndex(
        ...
        if (existingDeviceIndex !== -1) {
          ...
          console.log(
            `   - Old token: ${existingUser.fcmTokens[existingDeviceIndex].token.substring(0, 30)}...`
          );
      ...
      console.log(
        `📱 User ${existingUser._id} has ${existingUser.fcmTokens.length} active FCM token(s) after login`
      );
    }
