...
const createMatrimony = async (req, res) => {
  try {
    ...
    if (
      !payload.profileCreatedFor ||
      !payload.fullName ||
      !payload.dateOfBirth ||
      !payload.gender ||
      !payload.motherTongue ||
      !Array.isArray(payload.motherTongue) ||
      payload.motherTongue.length === 0 ||
      !payload.maritalStatus ||
      !payload.height ||
      !Array.isArray(payload.height) ||
      payload.height.length === 0 ||
      !payload.religion ||
      !Array.isArray(payload.religion) ||
      payload.religion.length === 0 ||
      !payload.caste ||
      !Array.isArray(payload.caste) ||
      payload.caste.length === 0 ||
      !payload.profession ||
      !Array.isArray(payload.profession) ||
      payload.profession.length === 0 ||
      !payload.highestQualification ||
      !Array.isArray(payload.highestQualification) ||
      payload.highestQualification.length === 0 ||
      !payload.employmentType ||
      !Array.isArray(payload.employmentType) ||
      payload.employmentType.length === 0 ||
      !payload.pincode ||
      !Array.isArray(payload.pincode) ||
      payload.pincode.length === 0 ||
      !payload.city ||
      !Array.isArray(payload.city) ||
      ...
      if (registeredPhone !== last10Digits) {
        ...
        if (!isVerified && req.user.role !== "ADMIN") {
          return res.status(400).json({
            message:
              "Contact number must be verified via OTP before creating matrimony profile. Please verify the phone number first.",
            status: 400,
            success: false,
            error: true,
            data: {
