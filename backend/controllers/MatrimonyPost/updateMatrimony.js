  ...
  try {
    ...
    if (payload.partnerAge) {
      if (!Array.isArray(payload.partnerAge) || payload.partnerAge.length === 0) {
        return res
          .status(400)
          .json({ message: "Partner age preferences must be a non-empty array" });
      }
      ...
      for (let ageRange of payload.partnerAge) {
        if (!ageRange.min || !ageRange.max) {
          return res
            .status(400)
            .json({ message: "Partner age range must have both min and max values" });
        }
        if (ageRange.min >= ageRange.max) {
          return res
            .status(400)
            .json({ message: "Partner minimum age must be less than maximum age" });
        }
      }
    }
    ...
    if (payload.partnerHeight) {
      if (!Array.isArray(payload.partnerHeight) || payload.partnerHeight.length === 0) {
        return res
          .status(400)
          .json({ message: "Partner height preferences must be a non-empty array" });
      }
      ...
      for (let heightRange of payload.partnerHeight) {
        if (!heightRange.min || !heightRange.max) {
          return res
            .status(400)
            .json({ message: "Partner height range must have both min and max values" });
        }
      }
    }
    ...
    if (payload.partnerMaritalStatus) {
      if (
        !Array.isArray(payload.partnerMaritalStatus) ||
        payload.partnerMaritalStatus.length === 0
      ) {
        return res
          .status(400)
          .json({ message: "Partner marital status must be a non-empty array" });
      }
    }
      ...
      if (registeredPhone !== last10Digits) {
        ...
        if (!isVerified && req.user.role !== "ADMIN") {
          return res.status(400).json({
            message:
              "Contact number must be verified via OTP before updating matrimony profile. Please verify the phone number first.",
            status: 400,
            success: false,
            error: true,
            data: {
