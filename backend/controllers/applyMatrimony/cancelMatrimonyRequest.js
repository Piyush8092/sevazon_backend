...
const cancelMatrimonyRequest = async (req, res) => {
  try {
    ...
    if (!ExistMatrimony) {
      return res
        .status(404)
        .json({ success: false, message: "Matrimony profile not found", data: null });
    }
    ...
    if (applicationIndex === -1) {
      return res
        .status(404)
        .json({ success: false, message: "No application found for this profile", data: null });
    }
    ...
    console.log(
      `[cancelMatrimonyRequest] Application cancelled: userId=${userId}, profileId=${matrimonyId}`
    );
    return res.json({
      success: true,
