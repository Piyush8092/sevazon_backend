...
exports.createOrGetRoom = async (req, res) => {
  try {
    ...
    if (!participantIds || !Array.isArray(participantIds) || participantIds.length < 2) {
      return res
        .status(400)
        .json({ message: "At least 2 participants required", success: false, error: true });
    }
    ...
    return res
      .status(500)
      .json({ message: "Failed to create/get room", data: e.message, success: false, error: true });
  }
};
...
exports.sendMessage = async (req, res) => {
  try {
    ...
    if (!roomId || !content) {
      return res
        .status(400)
        .json({ message: "roomId and content required", success: false, error: true });
    }
    ...
    return res
      .status(500)
      .json({ message: "Failed to send message", data: e.message, success: false, error: true });
  }
};
...
exports.getMessages = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Failed to fetch messages", data: e.message, success: false, error: true });
  }
};
...
exports.getRooms = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Failed to fetch rooms", data: e.message, success: false, error: true });
  }
};
