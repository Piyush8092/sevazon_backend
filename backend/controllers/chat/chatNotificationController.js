...
const sendMessageNotification = async (req, res) => {
  try {
    const { recipientId, senderName, messageText, conversationId, messageId } = req.body;
    ...
    const result = await notificationService.sendToUser(recipientId, title, body, data, options);
