...
const CreateAdd = async (req, res) => {
  try {
    ...
    if (
      !payload.adImages ||
      !Array.isArray(payload.adImages) ||
      payload.adImages.length < 1 ||
      payload.adImages.length > 5
    ) {
    ...
    if (payload.placementPages) {
      ...
      if (invalidPages.length > 0) {
        return res
          .status(400)
          .json({ message: `Invalid placement pages: ${invalidPages.join(", ")}` });
      }
    } else {
...
let getAllNotVerifiedAdds = async (req, res) => {
  try {
    ...
    res.json({
      message: "All ads are auto-approved. No unverified ads.",
      status: 200,
      data: [],
      success: true,
      error: false,
...
const FilterAdds = async (req, res) => {
  try {
    ...
    let result = await adModel.find(filter).populate("userId", "name email phone profileImage");
...
const specificAddAdminView = async (req, res) => {
  try {
    ...
    res.json({
      message: "Add detail retrieved successfully",
      status: 200,
      data: result,
      success: true,
      error: false,
...
const getUserAdsPlans = async (req, res) => {
  try {
    ...
    const user = await userModel.findById(userId).select("adPlansId").populate({
      path: "adPlansId.planId",
      model: "PricingPlan",
    });
    ...
    return res.status(200).json({
      message: "User Ads plans retrieved successfully",
      success: true,
      data: user.adPlansId,
    });
  } catch (error) {
