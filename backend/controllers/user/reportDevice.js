...
const reportDevice = async (req, res) => {
  try {
    ...
    if (existingTokenIndex !== -1) {
      ...
      const existingDeviceIndex = deviceId
      ...
      if (existingDeviceIndex !== -1) {
        ...
        console.log(
          `   - Old token: ${user.fcmTokens[existingDeviceIndex].token.substring(0, 30)}...`
        );
    ...
    console.log(
      `📊 Device report processed - User ${userId} now has ${user.fcmTokens.length} token(s)`
    );
