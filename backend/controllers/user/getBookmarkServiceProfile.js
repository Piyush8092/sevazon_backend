...
const getBookmarkServiceProfile = async (req, res) => {
  try {
    ...
    let result = await userModel.findById(userId).populate({
      ...
      select:
        "profileType profileImage yourName businessName selectCategory selectSubCategory area city state pincode timing experienceYear experienceMonth averageRating reviewsCount isVerified serviceType createdAt _id userId",
