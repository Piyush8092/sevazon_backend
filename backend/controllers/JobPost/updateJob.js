...
const updateJob = async (req, res) => {
  try {
    ...
    if (
      payload.workShift &&
      (!Array.isArray(payload.workShift) || payload.workShift.length === 0)
    ) {
    ...
    if (allowCallViaPhone === true) {
      ...
      const registeredPhone = (user.phone || "").toString().replace(/\D/g, "").slice(-10);
      ...
      if (registeredPhone !== last10Digits) {
        ...
        if (!isVerified && req.user.role !== "ADMIN") {
          return res.status(400).json({
            message:
              "Phone number must be verified via OTP before updating the job post. Please verify the phone number first.",
            status: 400,
            success: false,
            error: true,
            data: {
