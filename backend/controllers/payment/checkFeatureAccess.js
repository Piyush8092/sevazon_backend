...
const checkFeatureAccess = async (req, res) => {
  try {
    ...
    if (category === "post") {
      ...
      console.log(
        `   Free posts: ${user.freePostsUsed || 0}/${user.freePostLimit || 10} used, ${freePostsRemaining} remaining`
      );

      // If user has free posts remaining, grant access
      if (freePostsRemaining > 0) {
        console.log(
          `✅ Access granted via free post limit (${freePostsRemaining} free posts remaining)`
        );
    ...
    if (
      ...
      new Date(user.activePlan.endDate) > new Date()
    ) {
      ...
      const payments = await Payment.find(query).populate("planId").sort({ createdAt: -1 });
      const now = new Date();
